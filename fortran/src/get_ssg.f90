module get_ssg
  use comms, only:dp, lattice
  implicit none
  integer, parameter :: ssg_file_unit = 1245
  integer, parameter,public :: max_nsym = 2400
contains

  subroutine get_ssg_from_identifySSG(ssg_num,nsym,rotation,tau,spin_SU2,spin_rotation,time_reversal)
  ...
  ! and stores them in the provided arrays.
    ...
    character(len=15), intent(out) :: ssg_num
    ...
    open(unit=ssg_file_unit, file='irssg.in', form='unformatted', access='stream', status='old', action='read', iostat=ios)
    read(ssg_file_unit) nsym
    read(ssg_file_unit) ssg_num(1:nsym)
  ...
  subroutine output_ssg_operator(ssg_num,nsym,rotation,tau,spin_SU2,spin_rotation,time_reversal)
    implicit none
    ...
    character(len=15), intent(in) :: ssg_num
    character(len=4) :: tmp
    ...
    i = Verify(ssg_num,' ')
    if (i /= 0 .and. ssg_num(i:i) /= 'n') then
      write(*,'(A)')'It belongs to Spin Space Group (SSG) ' // trim(adjustl(ssg_num)) // ' .'
    endif
    write(tmp,'(I0)')nsym
