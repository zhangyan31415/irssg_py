...
subroutine get_comprel(num_litt_group,num_litt_group_unitary,litt_group,op_order,irrep_num,irrep_name,ch_table)
    ...
    if (relation == 2) then
        write(*,*) 'Compatibility relations: '
        do i = 1, irrep_num
            write(*,'(A)') trim(adjustl(irrep_name(i)))//'->'//trim(adjustl(irrep_name_old(i)))
        end do
...
subroutine decomposition_rep(num_litt_group1,num_litt_group2,num_litt_group_unitary_1,num_litt_group_unitary_2,op_order1,op_order2,irrep_num1,irrep_num2,irrep_name1,irrep_name2,ch_table1,ch_table2)
    ...
    do i=1, irrep_num2
        ...
        if (flag) then
            write(*,'(A)') trim(adjustl(irrep_name2(i)))//'->'//trim(adjustl(irrep_name1(j)))
        else
            do j1=1, irrep_num1
                do j2 = 1, irrep_num1
                    flag = .true.
            ...
            if (flag) then
                write(*,'(A)') trim(adjustl(irrep_name2(i)))//'->'//trim(adjustl(irrep_name1(j1)))//'+'//trim(adjustl(irrep_name1(j2)))
            else
                do j1=1, irrep_num1
                    do j2 = 1, irrep_num1
                        do j3 = 1, irrep_num1
